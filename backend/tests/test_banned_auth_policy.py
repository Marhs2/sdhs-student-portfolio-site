...
from backend.app.auth import get_current_profile, get_optional_profile  # noqa: E402
...
class BannedAuthPolicyTests(unittest.TestCase):
    ...
    def test_banned_profile_cannot_use_optional_authenticated_view(self) -> None:
        with (
            patch(
                "backend.app.auth.get_profile_by_email",
                return_value={
            ...
            with self.assertRaises(HTTPException) as context:
                get_optional_profile({"email": "banned@sdh.hs.kr"})

        self.assertEqual(context.exception.status_code, 403)

    def test_banned_admin_cannot_use_optional_private_profile_view(self) -> None:
        with (
            patch(
                "backend.app.auth.get_profile_by_email",
                return_value={
            ...
            with self.assertRaises(HTTPException) as context:
                get_optional_profile({"email": "banned-admin@sdh.hs.kr"})
